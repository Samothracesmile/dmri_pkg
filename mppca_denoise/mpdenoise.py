...
class MP(object):
    ...
    def process(self):
        ...
        Signal = np.zeros((sx, sy, sz, N))
        for nn in inputs:
            signal, sigma, npars = self.denoise(coords[:,nn], M, N, centering=centering)
            Sigma[x[nn], y[nn], z[nn]] = sigma
            Npars[x[nn], y[nn], z[nn]] = npars
            Signal[x[nn], y[nn], z[nn], :] = signal
