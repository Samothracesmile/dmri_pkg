...
with warnings.catch_warnings():
    ...
    import dipy.reconst.dki as reconst_dki
    import dipy.reconst.msdki as reconst_msdki
...
def refine_brainmask2(mask_img):
...
def dilate_mask(mask_file, new_mask_file, dilate_num=1):
    ...
    save_nifti(new_mask_file, mask_img, ref_fname=mask_file)

def fsl_skull_striping(b0_file, mask_file):
    '''
    Conduct skull striping for b0_file
    '''
    mask_cmd_str = f'bash /ifs/loni/faculty/shi/spectrum/yxia/code/src/fsl_skull_striping_b0.sh {b0_file} {mask_file}'
    os.system(mask_cmd_str)
...
def write_bvecs(bvec_file, bvecs):
    with open(bvec_file, 'w') as f:
        bvec_str = ('\n').join(str((' ').join( str(b) for b in bvec)) for bvec in bvecs.T)
    #     print(bvec_str)                   
        f.write(bvec_str)
